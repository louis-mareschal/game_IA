...
class Checkpointer:
    ...
    def end_generation(self, population):
        if (population.generation - self.last_generation_checkpoint) >= self.generation_interval:
    ...
    def save_checkpoint(self, population):
        ...
        if len(os.listdir(self.checkpoint_dir_path)) > 2:
            removed_checkpoint_name = sorted(os.listdir(self.checkpoint_dir_path), key=lambda name: int(name[11:]))[0]
